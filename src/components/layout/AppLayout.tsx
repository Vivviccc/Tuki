      ...
      <header className="sticky top-0 z-[1100] h-16 border-b border-slate-200/80 dark:border-slate-800/80 bg-white/80 dark:bg-[#090d16]/80 backdrop-blur-xl px-4 lg:px-8 flex items-center justify-between shadow-sm transition-colors duration-300">
        ...
        <div className="flex items-center gap-4">
          <Link to="/groups" className="flex items-center gap-2 group">
            <div className="w-9 h-9 rounded-xl bg-gradient-to-tr from-purple-600 to-indigo-600 flex items-center justify-center shadow-glow-brand transition-transform group-hover:scale-105">
          ...
          <div className="relative">
            <button
              ...
              className="flex items-center gap-2 px-3 py-1.5 rounded-xl bg-slate-100 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 hover:border-purple-400 text-xs font-semibold transition-colors"
            >
              <span className="w-2 h-2 rounded-full bg-emerald-500 animate-pulse" />
              <span className="max-w-[120px] sm:max-w-[180px] truncate font-bold text-slate-800 dark:text-slate-200">
            ...
            {isGroupDropdownOpen && (
              <div
                className="absolute left-0 mt-2 w-64 rounded-2xl bg-white dark:bg-[#111827] border border-slate-200 dark:border-slate-800 shadow-2xl p-2 z-50 animate-fade-in"
              ...
              >
                  ...
                  {groups.map((g) => (
                    <button
                      ...
                      className={`w-full flex items-center justify-between p-2 rounded-xl text-xs font-medium transition-colors ${
                        g.id === activeGroupId
                          ? 'bg-purple-50 dark:bg-purple-500/10 text-purple-600 dark:text-purple-400 font-bold border border-purple-200 dark:border-purple-500/30'
                ...
                <div className="pt-2 border-t border-slate-200 dark:border-slate-800 grid grid-cols-2 gap-1.5">
                  <button
                    onClick={() => {
                      setIsGroupDropdownOpen(false);
                      setIsCreateModalOpen(true);
                    }}
                    className="flex items-center justify-center gap-1 p-2 rounded-xl bg-purple-600 hover:bg-purple-700 text-[11px] font-bold text-white transition-colors"
                  >
                  ...
                  <button
                    ...
                    className="flex items-center justify-center gap-1 p-2 rounded-xl bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 text-[11px] font-bold text-slate-800 dark:text-slate-200 transition-colors"
                  >
                    <UserPlus className="w-3 h-3 text-purple-500" /> Join Code
                  </button>
                </div>
              </div>
            )}
        ...
        {activeGroupId && (
          <nav className="hidden md:flex items-center gap-1 bg-slate-100 dark:bg-slate-900/80 p-1 rounded-2xl border border-slate-200 dark:border-slate-800">
            <NavLink
        ...
        <div className="flex items-center gap-2 sm:gap-3">
          ...
          {activeGroupId && (
            <button
              ...
              className="flex items-center gap-1.5 px-3.5 py-2 rounded-xl bg-purple-600 hover:bg-purple-700 text-white text-xs font-bold shadow-glow-brand transition-all"
            >
